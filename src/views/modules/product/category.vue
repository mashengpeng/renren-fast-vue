<template>
  <div>
    ...
    <el-tree ref="menuTree" :allow-drop="allowDrop" :data="menus" :default-expanded-keys="defaultExpandKey"
             :draggable="true"
             :expand-on-click-node="false"
             :props="defaultProps" node-key="catId" show-checkbox
             @node-click="handleNodeClick">
            <span slot-scope="{ node, data }" class="custom-tree-node">
                <span>{{ node.label }}</span>
                <span>
                    <el-button v-if="node.level <= 2" size="mini" type="text" @click="() => append(data)">
                        Append
                    </el-button>
                    <el-button size="mini" type="text" @click="edit(data)">
                        Edit
                    </el-button>
